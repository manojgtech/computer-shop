...
<main class="category">
    <div class="category-header  d-flex justify-content-center align-items-center">
        <h3 class="my-4 display-5 fw-bold">{{$name}} Products</h3>
    </div>
    ...
    <div class="col-md-3 col-lg-3 col-sm-6">
        <div class="mmd-product-vertical">
            <div class="product-top">
                ...
                <a href="{{route("product",['slug'=>$product->slug])}}" target="_blank"><img lazy src="{{$product->defaultpic!=null ? url("brands/".$product->defaultpic) : url('brands/defaultpc.png')}}" alt="{{$product->title}}" class="img-fluid w-100 lazy"></a>
            </div>
            <div class="product-bottom">
                @if($product->category)
                <h4 class="mmd-product-category">{{ucwords($product->category->name)}}</a></h4>
                @endif
                <p class="mmd-product-name"><a href="{{route("product",['slug'=>$product->slug])}}" target="_blank">{{ucwords($product->title)}}</a></p>
